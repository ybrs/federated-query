...
NO DATACLASSES; state types are Pydantic models. Final, non-negotiable design decision: no Python dataclasses anywhere (they silently drop fields on reconstruction = wrong answer, no error). Every state type (plan/expression nodes, catalog/config/state objects) derives from `federated_query.model.StateModel` (a `pydantic.BaseModel` with `arbitrary_types_allowed`; construction is keyword-only - positional construction is intentionally NOT supported, since positional-by-field-order silently rebinds if a field is reordered). `StateModel` keeps mistakes loud: `extra="forbid"` (unknown construction kwarg raises) + a `model_copy` override that rejects unknown `update` keys. Migration COMPLETE; `tests/test_no_dataclasses.py` (AST-based) enforces no dataclasses, `tests/test_state_model.py` pins the loudness. Rules: never write a `@dataclass` or import `dataclasses` (new state types subclass `StateModel`); construct with keyword args only; fields are Pydantic annotations (no default = required); copy-with-change uses `node.model_copy(update={...})` - copies every field, overrides only what is named, never re-list fields into a raw constructor; equality/copy/introspection come from Pydantic (`==` is the complete change-detection contract, plus `model_copy`/`model_fields`); hold non-Pydantic values via `ConfigDict(arbitrary_types_allowed=True)`; nodes keep `children()`/`with_children()` (= `model_copy(update=...)`)/`accept()`/`schema()`. `tests/test_node_field_preservation.py` pins that `with_children` preserves every field.

Mutable by default; do not reach for immutability. Use plain mutable classes. Do NOT make a type immutable (no `frozen=True`, no `__setattr__` guards, no read-only wrappers) unless there is a STRONG, concrete reason; immutability is not a goal in itself and does not by itself guarantee correctness. Never introduce it speculatively or change a design to be immutable without a stated reason. Any type that genuinely must be immutable, and why, MUST be listed as an exception in CLAUDE.md (currently none).
