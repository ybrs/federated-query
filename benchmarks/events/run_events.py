...
def ev_cte(parquet, where=None, time_from=None, time_to=None):
    """The filtered event-stream CTE every oracle starts from."""
    clauses = []
    if where:
        clauses.append(where)
    if time_from:
        clauses.append("ts >= TIMESTAMP '%s'" % time_from)
    if time_to:
        clauses.append("ts < TIMESTAMP '%s'" % time_to)
    predicate = (" WHERE " + " AND ".join(clauses)) if clauses else ""
    return (
        "ev AS (SELECT entity_id AS actor, ts, seq AS tb, event_name AS name, "
        "device, country FROM read_parquet('%s')%s)" % (parquet, predicate)
    )
...
def paths_oracle(con, parquet, depth, top, anchor=None, direction="forward",
    ...
    anchor variant. Share divides by the pre-limit window total."""
    base = ev_cte(parquet, where, time_from, time_to)
    ...
    if direction == "forward":
        ...
        strung = ("strung AS (SELECT actor, name, %s FROM collapsed "
                  "WINDOW w AS (PARTITION BY actor, ses ORDER BY ts, tb))"
                  % ", ".join(lead_cols))
        path = "name"
        for i in range(1, depth):
