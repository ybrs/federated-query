...
pub struct PathsSpec {
    ...
    /// Window length in events; at least 2, unbounded above.
    pub depth: u32,
    ...
    /// ANCHOR semantics: the range admits a window when its ANCHOR event
    /// (first event; the ENDING AT anchor for backward flows) falls inside
    /// it - the window then follows the actor past the range end. Events are
    /// never dropped from the stream by the range.
    pub range: TimeRange,
    /// ANCHOR semantics, like `range`: the predicate is tested on the
    /// window's anchor event only, never dropping events from the stream.
    pub filter: Option<EventPredicate>,
}
...
impl Default for EventsConfig {
    ...
    /// grains, an 8 GiB warm cache, and a 16 GiB query budget (a deep paths
    /// scan holds one 16-byte fingerprint per distinct window, about 4.5 GiB
    /// at 100M events).
    fn default() -> Self {
        Self {
            ...
            query_memory_bytes: 16 * 1024 * 1024 * 1024,
            max_generations: 8,
