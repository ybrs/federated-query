...
use crate::build::{
    append_frame_reader, lookup_prior, merge_max, merge_min, write_shard_files, ActorTable,
    BuildParams, IdAssignment, PropRowCell, ShardResult,
};
...
pub(crate) fn finalize_external(
    ...
    let assignment = assign_ids_streaming(props, actor_table, priors, spill_path, shard as u32)?;
...
fn assign_ids_streaming(
    ...
    let mut present = vec![false; actor_table.len()];
    append_frame_reader(spill_path, props, |chunk| {
        for row in 0..chunk.rows() {
            present[chunk.actor(row) as usize] = true;
        }
        Ok(())
    })?;
    ...
    for (code, seen) in present.iter().enumerate() {
        if *seen {
            ...
            pairs.push((actor_table.hash_of(code), actor_table.key(code).to_vec()));
        }
    }
    pairs.sort_unstable();
...
fn write_sorted_runs(
    ...
    append_frame_reader(spill_path, props, |chunk| {
        for row in 0..chunk.rows() {
            ...
            let code = chunk.actor(row);
            let key = actor_table.key(code).to_vec();
            let local = resolve_local(assignment, priors, actor_table.hash_of(code), &key);
            let run_row = RunRow {
                local,
                ts,
                tiebreak,
                event,
                cells: chunk.cells(promoted, promoted_values, row),
                key,
            };
