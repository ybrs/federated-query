...
impl StoreIo {
    ...
    /// Write a complete file and read it back, raising unless the bytes on
    /// the far side hash identically. One rewrite is attempted first: the
    /// mismatch this guards against is silent in-memory corruption of the
    /// outgoing buffer or the write path, and publishing such a file would
    /// surface later as a checksum crash on some unlucky query.
    pub fn put_verified(&self, rel: &str, bytes: Vec<u8>) -> Result<(), EventStoreError> {
        let expected = xxhash_rust::xxh3::xxh3_64(&bytes);
        // A refcounted payload makes the retry free of copies.
        let payload = bytes::Bytes::from(bytes);
        let path = StorePath::from(rel);
        for attempt in 0..2 {
            self.runtime
                .block_on(self.store.put(&path, PutPayload::from(payload.clone())))
                .map_err(|error| Self::io_error(rel, &error))?;
            let readback = xxhash_rust::xxh3::xxh3_64(&self.get(rel)?);
            ...
            if attempt == 1 {
                return Err(EventStoreError::Corrupt {
                    file: rel.to_string(),
                    detail: format!(
                        "written bytes hash {expected:016x} but read back {readback:016x}                          twice; the write path is corrupting data"
                    ),
                });
            }
        }
        unreachable!("the verify loop returns on success or raises on the retry");
    }
    ...
    /// Whether `rel` exists, from one metadata stat (never a listing).
    pub fn exists(&self, rel: &str) -> Result<bool, EventStoreError> {
        let path = StorePath::from(rel);
        match self.runtime.block_on(self.store.head(&path)) {
            Ok(_) => Ok(true),
            Err(object_store::Error::NotFound { .. }) => Ok(false),
            Err(error) => Err(Self::io_error(rel, &error)),
        }
    }
...
pub enum CachedItem {
    ...
    PathsSa(crate::paths_sa::PathsSa),
}
...
impl CachedItem {
    ...
    fn heap_bytes(&self) -> usize {
        match self {
            ...
            CachedItem::PathsSa(index) => index.heap_bytes(),
...
pub struct BlockCache {
    ...
    /// Per-key load gates: one loader per missing item, racers wait on it.
...
impl BlockCache {
    ...
    fn get_or_load(
        ...
        // The load runs outside the map lock so concurrent shard workers
        // decode in parallel; a per-key gate single-flights it, so racers for
        // the SAME item (a query landing during the paths-index prewarm) wait
        // for the loader instead of duplicating a multi-gigabyte read.
        let gate = {
            let mut loading = self.loading.lock().expect("load gate map poisoned");
            Arc::clone(loading.entry(key.clone()).or_insert_with(|| Arc::new(Mutex::new(()))))
        };
        let _held = gate.lock().expect("load gate poisoned");
        {
            let mut map = self.inner.lock().expect("event cache lock poisoned");
            if let Some(entry) = map.get_mut(&key) {
                entry.last_used = tick;
        ...
        let loaded = load();
        self.loading.lock().expect("load gate map poisoned").remove(&key);
...
/// The cached paths index stored at `rel`, loading with `load` on a miss
/// (the same global LRU the block/preagg reads share).
pub(crate) fn cached_paths_sa(
    io: &StoreIo,
    rel: &str,
    budget: u64,
    load: impl FnOnce() -> Result<CachedItem, EventStoreError>,
) -> Result<Arc<CachedItem>, EventStoreError> {
    let name: Arc<str> = Arc::from(io.root().join(rel).to_string_lossy().as_ref());
    let key = CacheKey::new(&name, 0xf6, u32::MAX, 0);
    BlockCache::global().get_or_load(key, budget, load)
}
