...
/// Parse `SHOW EVENTS FROM <dataset>` (`IN` is accepted for `FROM`): the
/// distinct event names of one dataset's dictionary.
pub(crate) fn classify_show_events<'a>(
    tokens: &mut Tokenizer<'a>,
) -> Result<Statement<'a>, ParseError> {
    tokens.expect_keyword("SHOW")?;
    tokens.expect_keyword("EVENTS")?;
    if !(tokens.peek_word_is("FROM") || tokens.peek_word_is("IN")) {
        return Err(ParseError::Parse(
            "SHOW EVENTS needs a dataset: SHOW EVENTS FROM <dataset>".to_string(),
        ));
    }
    tokens.next_token();
    let dataset = view_name(tokens, "event dataset")?;
    expect_end(tokens, "SHOW EVENTS FROM <dataset>")?;
    Ok(Statement::ShowEvents { dataset })
}
...
mod tests {
    ...
    #[test]
    fn show_events_needs_a_dataset_and_classifies_with_one() {
        for sql in ["SHOW EVENTS FROM web", "show events in Web"] {
            assert_eq!(
                classify(sql),
                Statement::ShowEvents {
                    dataset: "web".to_string()
                },
                "{sql}"
            );
        }
        assert!(classify_statement("SHOW EVENTS").is_err());
        assert!(classify_statement("SHOW EVENTS FROM web extra").is_err());
    }
