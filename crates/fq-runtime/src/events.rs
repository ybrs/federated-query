...
fn events_root_path(source_path: &str) -> std::path::PathBuf {
...
impl Runtime {
    ...
    pub(crate) fn create_event_dataset(
        ...
        status_result(&format!(
            ...
            if report.promoted.is_empty() {
                ...
                format!(", promoted to raw strings: {}", report.promoted.join(", "))
            }
        ))
    }
    ...
    pub(crate) fn refresh_event_dataset(
        ...
        let store = self.event_store()?;
        let record = store.dataset(name)?.record;
...
/// The tracked watermark value of a stream.
...
fn watermark_literal(stored: &str, dataset: &str) -> Result<String, RuntimeError> {
    let value: WatermarkValue = serde_json::from_str(stored).map_err(|error| {
        RuntimeError::EventStatement(format!(
    ...
    Ok(match value {
        ...
        WatermarkValue::Text(text) => format!("'{}'", text.replace('\'', "''")),
    })
}

/// A batch source over the streaming connector pull that tracks the maximum
/// refresh-key value it saw (the next watermark).
struct StreamSource {
    stream: connectors::FetchStream,
    watermark_column: Option<(usize, String)>,
    watermark: Option<WatermarkValue>,
}
...
impl StreamSource {
    ...
    /// Fold one batch's refresh-key column into the tracked maximum.
    fn track(&mut self, batch: &RecordBatch) -> Result<(), String> {
        let Some((position, name)) = &self.watermark_column else {
            return Ok(());
        };
        let array = batch.column(*position);
        if array.null_count() > 0 {
            return Err(format!(
                "refresh_key column '{name}' has NULL values; a null cannot carry the \
                 watermark"
            ));
        }
        ...
        if let Some(values) = any.downcast_ref::<Int64Array>() {
            for index in 0..values.len() {
                ...
                match &self.watermark {
                    Some(WatermarkValue::Int(current)) if *current >= value => {}
                    Some(WatermarkValue::Text(_)) => {
                        return Err(format!(
                            "refresh_key column '{name}' changed type since the last pull"
                        ))
                    }
                    Some(WatermarkValue::Int(_)) | None => {
                        self.watermark = Some(WatermarkValue::Int(value));
                    }
                }
            }
            return Ok(());
        ...
        if let Some(values) = any.downcast_ref::<StringArray>() {
            for index in 0..values.len() {
                let value = values.value(index);
                match &self.watermark {
                    Some(WatermarkValue::Text(current)) if current.as_str() >= value => {}
                    Some(WatermarkValue::Int(_)) => {
                        return Err(format!(
                            "refresh_key column '{name}' changed type since the last pull"
                        ))
                    }
                    Some(WatermarkValue::Text(_)) | None => {
                        self.watermark = Some(WatermarkValue::Text(value.to_string()));
                    }
                }
            }
            return Ok(());
        ...
        Err(format!(
            "refresh_key column '{name}' has type {}, which cannot carry a watermark; \
             use an integer or text column",
            array.data_type()
        ))
    }
}
...
/// Map a paths result onto Arrow.
fn paths_to_arrow(
    result: &fq_events::PathsResult,
) -> Result<(SchemaRef, Vec<RecordBatch>), RuntimeError> {
    let schema = Arc::new(Schema::new(vec![
        Field::new("rank", ArrowDataType::Int32, false),
    ...
    let columns: Vec<Arc<dyn arrow::array::Array>> = vec![
        Arc::new(Int32Array::from(
            result
                .rows
                .iter()
                .map(|row| i32::try_from(row.rank).expect("rank fits i32"))
                .collect::<Vec<_>>(),
        )),
        ...
        Arc::new(Int64Array::from(
            result
                .rows
                .iter()
                .map(|row| i64::try_from(row.occurrences).expect("count fits i64"))
                .collect::<Vec<_>>(),
        )),
        Arc::new(Float64Array::from(
            result.rows.iter().map(|row| row.share).collect::<Vec<_>>(),
        )),
    ];
    let batch = RecordBatch::try_new(Arc::clone(&schema), columns)
        .map_err(|error| RuntimeError::ResultShape(error.to_string()))?;
...
pub(crate) fn paths_describe_columns() -> Vec<(String, DataType)> {
    vec![
        ("rank".to_owned(), DataType::Integer),
        ("path".to_owned(), DataType::Varchar),
