...
def test_paths_matches_the_oracle(engine):
    ...
    """, [parquet]).fetchall()
    # The engine returns one row per step; rebuild each ranked path string
    # from its path_id group (seq rows are emitted in ascending order).
    paths = {}
    for index in range(len(table["path_id"])):
        path_id = table["path_id"][index]
        entry = paths.setdefault(path_id, ([], table["occurrences"][index]))
        entry[0].append(table["event"][index])
    got = []
    for path_id in sorted(paths):
        steps, occurrences = paths[path_id]
        got.append((" -> ".join(steps), occurrences))
    assert got == expected
...
def test_invalid_statements_raise(engine):
    ...
    with pytest.raises(Exception, match="DEPTH"):
        run(runtime, "PATHS ON web DEPTH 1")
    # DEPTH has no upper bound; a depth past every stream length is a valid
    # statement with an empty answer, never an error.
    deep = run(runtime, "PATHS ON web DEPTH 99").to_pydict()
    assert deep["path_id"] == []
