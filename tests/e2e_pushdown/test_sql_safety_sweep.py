...
REJECTED_SQL = [
    f"SELECT region, SUM(price) FROM {TABLE} GROUP BY ROLLUP (region)",
    f"SELECT region, status, SUM(price) FROM {TABLE} GROUP BY CUBE (region, status)",
    f"SELECT region, SUM(price) FROM {TABLE} GROUP BY GROUPING SETS ((region), ())",
