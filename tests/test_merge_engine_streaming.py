...
class _CountingNode:
    ...
    def __init__(self, schema: pa.Schema, batch_count: int):
        """Hold the schema and how many one-row batches to yield."""
        self._schema = schema
    ...
    def execute(self):
        ...
        for value in range(self._batch_count):
            self.pulled += 1
            yield pa.record_batch(
                {self._schema.names[0]: pa.array([value], self._schema.types[0])}
            )
...
def test_probe_input_is_a_streaming_reader_not_a_table():
    ...
    build = _CountingNode(schema, batch_count=1)
    probe = _CountingNode(schema, batch_count=5)
